...
│   ├ test22.py
└ test.py
/my-testcases> icedtea --tc test3
...
/my-testcases> icedtea --tcdir folder2 --tc test22
...
```

## Development

Install `IcedTea` in development mode:
...
and it can be given to IcedTea with command line
...
| --version | Print IcedTea version and exit|  |  |  |
...
| --failure_return_value | Set IcedTea to return a failing code to caller if one or more tests fail during the run. Otherwise return value will always be 0 |  |  |  |
| --ignore_invalid_params | Ignore parameters MIcedTea cannot parse instead of stopping the run (for backwards compatibility) |  |  |  |
