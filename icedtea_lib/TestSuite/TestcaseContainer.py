...
from icedtea_lib.bench import ReturnCodes
from icedtea_lib.tools.tools import get_fw_version, import_module, loadClass
from icedtea_lib.arguments import get_parser, get_tc_arguments, get_base_arguments
...
class TestcaseContainer(object):
    ...
    def run(self, forceflash=False):
        ...
        if len(unknown) > 0:
            for para in unknown:
                self.logger.warning("IcedTea received unknown parameter {}".format(para))
    ...
    def _check_version(self, tc_instance):
        if tc_instance.config.get("compatible") and tc_instance.config['compatible']['framework']['name']:
            ...
            if framework['version'] and framework['name'] == "IcedTea":
                ...
                try:
                    if not self._check_major_version(fw_version, ver_str):
                        result = self._wrong_version(tc_instance, ver_str,
                                                     ...
                                                     "Please install IcedTea {}".format(ver_str))
                        return result
                except ValueError:
    ...
    def _wrong_version(self, tc_instance, ver_str, msg=None):
        msg = msg if msg else "Version {} of IcedTea required".format(ver_str)
        self.logger.info("TC '%s' will be skipped because of '%s'" % (
