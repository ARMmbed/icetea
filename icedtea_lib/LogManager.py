...
classes and helpers used by IcedTea.
"""
...
def init_base_logging(directory="./log", verbose=False, silent=False, color=False, no_file=False,
                      truncate=True):
    """ Initialize the IcedTea logging by creating a directory to store logs
    for this run and initialize the console logger for IcedTea itself
    ...
    # Initialize the simple console logger for IcedTeaManager
    icedtealogger = logging.getLogger("icedtea")
    icedtealogger.setLevel(logging.DEBUG)
    ...
    VERBOSE_ON = verbose
    if not no_file:
        file_handler = logging.FileHandler(get_base_logfilename("icedtea.log"))
        file_handler.setFormatter(formatter)
        file_handler.setLevel(logging.DEBUG)
        icedtealogger.addHandler(file_handler)
    if verbose:
    ...
    icedtealogger.addHandler(stream_handler)
    TRUNCATE_LOG = truncate
    if TRUNCATE_LOG:
        icedtealogger.addFilter(ContextFilter())
