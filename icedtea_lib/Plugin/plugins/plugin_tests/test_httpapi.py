...
from icedtea_lib.Plugin.plugins.HttpApi import Api as tc_api
...
class APITestCase(unittest.TestCase):
    ...
    @mock.patch("icedtea_lib.tools.HTTP.Api.HttpApi.get", side_effect=iter([
        MockedRequestsResponse(
        status_code=200), MockedRequestsResponse(status_code=404),
        MockedRequestsResponse(status_code=404)]))
    @mock.patch("icedtea_lib.tools.HTTP.Api.HttpApi.put", side_effect=iter([
        MockedRequestsResponse(
        status_code=200), MockedRequestsResponse(status_code=404),
        MockedRequestsResponse(status_code=404)]))
    @mock.patch("icedtea_lib.tools.HTTP.Api.HttpApi.post", side_effect=iter([
        MockedRequestsResponse(
        status_code=200), MockedRequestsResponse(status_code=404),
        MockedRequestsResponse(status_code=404)]))
    @mock.patch("icedtea_lib.tools.HTTP.Api.HttpApi.delete", side_effect=iter([
        MockedRequestsResponse(
        status_code=200), MockedRequestsResponse(status_code=404),
        MockedRequestsResponse(status_code=404)]))
    @mock.patch("icedtea_lib.tools.HTTP.Api.HttpApi.patch", side_effect=iter([
        MockedRequestsResponse(
        status_code=200), MockedRequestsResponse(status_code=404),
        MockedRequestsResponse(status_code=404)]))
