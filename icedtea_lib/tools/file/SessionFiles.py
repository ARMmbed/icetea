...
from icedtea_lib.tools.file import FileUtils
...
class JsonFile(object):
    ...
    def write_file(self, content, filepath=None, filename=None, indent=None, keys_to_write=None):
        ...
        name = self._endsWith(name, ".json")
        path = self._endsWith(path, os.path.sep)
    ...
    def write_values(self, data, filepath=None, filename=None, indent=None, keys_to_write=None):
        ...
        Creates filename.temp with updated content, removes the old file and finally renames the .temp to match the old file.
        ...
        try:
            ...
            FileUtils.removeFile(name, path)
            FileUtils.renameFile(name + '.temp', name, path)
            return os.path.join(path, name)
        except EnvironmentError as e:
