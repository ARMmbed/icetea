...
from icedtea_lib.TestSuite.TestSuite import TestSuite, SuiteException
from icedtea_lib.TestSuite.TestcaseContainer import TestStatus
from icedtea_lib.ResultList import ResultList
from icedtea_lib.arguments import get_base_arguments, get_parser, get_tc_arguments
from icedtea_lib.tools.tools import Singleton, get_fw_version
from icedtea_lib.cloud import Cloud
from icedtea_lib.Plugin.PluginManager import PluginManager


"""
    IcedTeaManager module. Contains IcedTeaManager class and some helpers.
    IcedTeaManager is the handler of the entire run. It determines return codes
...
class TCMetaSchema(object):
    ...
    def __init__(self, libpath="./icedtea_lib"):
...
class IcedTeaManager(object):
    """
        IcedTeaManager class. This is the master of the entire run. The primary entry point into
        execution is the run method.
    """
    def __init__(self):
        """
        Constructor for IcedTeaManager. Appends libraries to sys.path, loads the test case
        ...
        self.args, self.unknown = IcedTeaManager._parse_arguments()
        ...
        self.logger = LogManager.get_logger("IcedTea")
    ...
    def check_args(self):
        ...
        if len(sys.argv) < 2:
            self.logger.error("IcedTea called with no arguments! ")
            parser.print_help()
            return False
    ...
    def run(self, args=None):
        ...
        if self.args.version and version:
            ...
            print("Unable to get version. Have you installed IcedTea correctly?")
            return retcodesummary

        self.logger.info("Using IcedTea version {}".format(version) if version
                         else "Unable to get IcedTea version. Is IcedTea installed?")
    ...
    def _cleanlogs(self, silent=False):
        ...
        try:
            print("cleaning up IcedTea log directory.")
